...
namespace MoonyLeeKicks
{
    public static class WardManager
    {
        ...
        public static void CastWardTo(Vector3 position)
        {
            if (CanWardJump)
            {
                var endPos = MyHero.Position +
                             (position - MyHero.Position).Normalized() *
                             Math.Min(WardRange, MyHero.Distance(position));
                var ward = GetItem;
                if (ward != null)
                {
                    ward.Cast(endPos);
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe)
            {
                if (args.SData.Name.Equals(MyHero.Spellbook.GetSpell(SpellSlot.W).SData.Name) &&
        ...
        {
            return sender is Obj_AI_Minion && sender.Team == MyHero.Team && sender.Name != null &&
        ...
        public static Obj_AI_Minion GetNearestTo(Vector3 position)
        {
            return
                _wardsAvailable.Where(
                    m =>
                        m.IsValid && !m.IsDead &&
                        MyHero.Distance(m, true) <= Math.Pow(SpellManager.W1.Range + 120f, 2))
